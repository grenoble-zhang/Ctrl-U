            ...
            with accelerator.accumulate(controlnet):
                ...
                if args.rflow:
                    ...
                    if args.uncertainty:
                        ...
                        os.makedirs(f"work_dirs/measure_uncertainty/Captioned_ADE20K/denoised_{step_number}_{args.add_timestep}", exist_ok=True)
                        for i in range(primary_img.size(0)):
                            img = to_pil(primary_img[i])
                            # 构建文件名
                            filename = f"work_dirs/measure_uncertainty/Captioned_ADE20K/denoised_{step_number}_{args.add_timestep}/{img_number + i}_{step_number}_.png"
                            img.save(filename)

                        for i in range(auxiliary_img.size(0)):
                            img = to_pil(auxiliary_img[i])
                            # 构建文件名
                            filename = f"work_dirs/measure_uncertainty/Captioned_ADE20K/denoised_{step_number}_{args.add_timestep}/{img_number + i}_{step_number-args.add_timestep}_.png"
                            img.save(filename)
