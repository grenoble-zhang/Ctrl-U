...
def log_validation(
    ...
    if args.rflow:
        pipeline.scheduler = rectified_flow.FlowMatchEulerDiscreteScheduler(
    ...
    ), "Validation dataset is required for logging validation images."
    try:
        ...
        validation_conditions = [
            item[conditioning_image_column] for item in val_dataset
        ]
    ...
    if args.conditioning_image_column == "canny":
        ...
        with autocast():
            validation_conditions = [
                torchvision.transforms.functional.pil_to_tensor(x)
                for x in validation_conditions
            ]
            validation_conditions = [x / 255.0 for x in validation_conditions]
            validation_conditions = kornia.filters.canny(
                torch.stack(validation_conditions), low_threshold, high_threshold
            )[1]
            validation_conditions = torch.chunk(
                validation_conditions, len(validation_conditions), dim=0
            )
            validation_conditions = [
                torchvision.transforms.functional.to_pil_image(x.squeeze(0), "L")
                for x in validation_conditions
            ]
        ...
        with autocast():
            ...
            validation_conditions = [
                torchvision.transforms.functional.resize(
                    x, (512, 512), interpolation=Image.BICUBIC
                )
                for x in validation_conditions
            ]
            with torch.no_grad():
                validation_conditions = reward_model(
                    torch.stack(validation_conditions).to(accelerator.device)
                )
            validation_conditions = (
                1 - validation_conditions
    ...
    for validation_prompt, validation_condition, validation_image in zip(
        validation_prompts, validation_conditions, validation_images
    ):
        if val_dataset is not None:
            validation_image = validation_image.convert("RGB").resize(
                (512, 512), Image.Resampling.BICUBIC
            )
            validation_condition = validation_condition.convert("RGB").resize(
                (512, 512), Image.Resampling.BICUBIC
            )
        else:
            validation_condition = (
                Image.open(validation_condition)
                .convert("RGB")
        ...
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                formatted_images.append(
                    wandb.Image(validation_image, caption="Controlnet input image")
                )
                formatted_images.append(
                    wandb.Image(validation_condition, caption="Controlnet conditioning")
                )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help=("Max number of samples for validation during training, default to 10"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--step_number", type=int, default=1
    )
    ...
    parser.add_argument("--tune_sd", action="store_true")
...
def make_train_dataset(args, tokenizer, accelerator, split="train"):
    ...
    if args.dataset_name is not None:
        ...
        if args.dataset_name.count("/") == 1:
            ...
            if "Captioned_ADE20K" in args.dataset_name or "Captioned_COCOStuff" in args.dataset_name:
                dataset = load_dataset(
                    "parquet", data_files=f"{args.dataset_name}/validation-*.parquet"
                )
            else:
                dataset = load_dataset(
                    "parquet", data_files=f"{args.dataset_name}/*.parquet"
                )
    else:
    ...
    def preprocess_train(examples):
        ...
        if args.label_column is not None:
            dtype = torch.long
            labels = [
                torch.tensor(np.asarray(label), dtype=dtype).unsqueeze(0)
                for label in examples[args.label_column]
            ]
        ...
        if args.label_column is not None:
            grouped_data = [
                torch.cat([x, y, z])
                for (x, y, z) in zip(images, conditioning_images, labels)
            ]
            ...
            grouped_data = [
                torch.cat([x, y]) for (x, y) in zip(images, conditioning_images)
            ]
            grouped_data = group_random_crop(grouped_data, args.resolution)
        ...
        for i, img_condition in enumerate(conditioning_images):
            ...
            if rand_num < args.image_condition_dropout:
                ...
                rand_num
                < args.image_condition_dropout
                + args.text_condition_dropout
                + args.all_condition_dropout
            ):
                conditioning_images[i] = torch.zeros_like(img_condition)
                examples[caption_column][i] = ""
...
def main(args):
    ...
    if args.rflow:
        noise_scheduler = rectified_flow.FlowMatchEulerDiscreteScheduler(
            num_train_timesteps=1000, shift=3.0
        )
    ...
    if args.use_ema:
        ema_controlnet = ControlNetModel.from_pretrained(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        loss_per_epoch = 0.0
        pretrain_loss_per_epoch = 0.0
        reward_loss_per_epoch = 0.0
        reward_wo_loss_per_epoch = 0.0
        variance_per_epoch = 0.0
        exp_variance_per_epoch = 0.0

        train_loss, train_pretrain_loss, train_reward_loss, train_reward_wo_loss = (
            0.0,
            0.0,
            0.0,
            0,
        )
        train_variance, train_exp_variance = 0.0, 0.0
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                if args.conditioning_image_column == "canny":
                    low_threshold = 0.1  # low_threshold = random.uniform(0, 1)
                    high_threshold = (
                        0.2  # high_threshold = random.uniform(low_threshold, 1)
                    )
                    with torch.no_grad():
                        ...
                        labels, controlnet_image = reward_model(
                            denormalized_condition_image, low_threshold, high_threshold
                        )
                        controlnet_image = controlnet_image.expand(
                            -1, 3, -1, -1
                        )  # (B, 3, H, W)
                elif args.conditioning_image_column in ["lineart", "hed"]:
                    with torch.no_grad():
                        ...
                        denormalized_condition_image = controlnet_image * std + mean
                        labels = reward_model(
                            denormalized_condition_image.to(weight_dtype)
                        )
                        controlnet_image = labels.expand(-1, 3, -1, -1)  # (B, 3, H, W)
                        controlnet_image = (
                            1 - controlnet_image
                            if args.task_name == "lineart"
                            else controlnet_image
                        )
                ...
                if args.rflow:
                    ...
                    sigmas = rectified_flow.get_sigmas(
                    ...
                    weighting = rectified_flow.compute_loss_weighting_for_sd3(
                    ...
                    if args.task_name == "canny":
                        outputs = reward_model(
                            image.to(accelerator.device), low_threshold, high_threshold
                        )
                    else:
                    ...
                    if (
                        ...
                        outputs = torchvision.transforms.functional.resize(
                            outputs,
                            (args.resolution, args.resolution),
                            interpolation=transforms.InterpolationMode.BILINEAR,
                        )
                        max_values = (
                            outputs.view(args.train_batch_size, -1)
                            .amax(dim=1, keepdim=True)
                            .view(args.train_batch_size, 1, 1)
                        )
                    ...
                    if args.task_name == "segmentation":
                        ...
                        labels = label_transform(
                            labels, args.task_name, args.dataset_name
                        )
                    else:
                        raise NotImplementedError(
                            f"Not support task: {args.task_name}."
                        )

                    labels = (
                        [x.to(accelerator.device) for x in labels]
                        if isinstance(labels, list)
                        else labels.to(accelerator.device)
                    )

                    # Determine which samples in the current batch need to calculate reward loss
                    timestep_mask = (
                        args.min_timestep_rewarding <= timesteps.reshape(-1, 1)
                    ) & (timesteps.reshape(-1, 1) <= args.max_timestep_rewarding)

                    # calculate the reward loss
                    reward_loss = get_reward_loss(
                        output_primary, labels, args.task_name, reduction="none"
                    )  # B, H, W
                    ...
                    if args.uncertainty:
                        ...
                        bsz = latents.shape[0]
                        # Sample a random timestep for each image
                        timesteps = torch.full((bsz,), step_number, device=latents.device)
                        ...
                        # timesteps[timesteps > 999] -= 2 * args.add_timestep
                        ...
                        # os.makedirs("work_dirs/measure_uncertainty/Captioned_ADE20K/origin", exist_ok=True)
                        # for i in range(controlnet_image.size(0)):
                        #     img = to_pil(controlnet_image[i])
                        #     # 构建文件名
                        #     filename = f"work_dirs/measure_uncertainty/Captioned_ADE20K/origin/{img_number + i}.png"
                        #     img.save(filename)
                        ...
                        primary_img = image[: args.train_batch_size]
                        auxiliary_img = image[args.train_batch_size :]

                        os.makedirs(f"work_dirs/measure_uncertainty/Captioned_ADE20K-ours/denoised_{step_number}_{args.add_timestep}", exist_ok=True)
                        for i in range(primary_img.size(0)):
                            img = to_pil(primary_img[i])
                            # 构建文件名
                            filename = f"work_dirs/measure_uncertainty/Captioned_ADE20K-ours/denoised_{step_number}_{args.add_timestep}/{img_number + i}_{step_number}_.png"
                            img.save(filename)

                        for i in range(auxiliary_img.size(0)):
                            img = to_pil(auxiliary_img[i])
                            # 构建文件名
                            filename = f"work_dirs/measure_uncertainty/Captioned_ADE20K-ours/denoised_{step_number}_{args.add_timestep}/{img_number + i}_{step_number-args.add_timestep}_.png"
                            img.save(filename)
                            
                        img_number += primary_img.size(0)
                        ...
                        variance = (
                            torch.sum(
                                kl_distance(
                                    uncertainty_log_sm(output_primary),
                                    uncertainty_sm(output_auxiliary),
                                ),
                                dim=1,
                            )
                            * args.uncertainty_scale
                        )  # TODO check mean(variance) -> 0

                        # variance = (torch.sum(kl_distance(uncertainty_log_sm(output_primary),uncertainty_sm(output_auxiliary)), dim=1)
                        #             + torch.sum(kl_distance(uncertainty_log_sm(output_primary),uncertainty_sm(output_auxiliary)), dim=1))
                        # * 0.5 * args.uncertainty_scale # TODO check mean(variance) -> 0

                        exp_variance = torch.exp(
                            -variance
                        )  # B,W,H  TODO check mean(variance) -> 1
                        # loss = torch.mean(loss*exp_variance) + torch.mean(variance)

                        # calculate the reward loss
                        reward_wo_loss = get_reward_loss(
                            output_primary, labels, args.task_name, reduction="none"
                        )  # B,W,H
                        ...
                        if args.task_name == "segmentation":
                            ...
                            reward_wo_loss = (background_mask * reward_wo_loss).sum(
                                dim=(-1, -2)
                            ) / (background_mask.sum(dim=(-1, -2)) + 1e-10)
                        elif args.task_name == "canny":
                            pass
                        ...
                        reward_wo_loss = (timestep_mask * reward_wo_loss).sum() / (
                            timestep_mask.sum() + 1e-10
                        )
                        ...
                        # calculate the reward loss
                        reward_loss = get_reward_loss(
                            outputs, labels, args.task_name, reduction="none"
                        )
                ...
                avg_pretrain_loss = accelerator.gather(
                    pretrain_loss.repeat(args.train_batch_size)
                ).mean()
                avg_reward_loss = accelerator.gather(
                    reward_loss.repeat(args.train_batch_size)
                ).mean()
                avg_reward_wo_loss = accelerator.gather(
                    reward_wo_loss.repeat(args.train_batch_size)
                ).mean()
                # pdb.set_trace()
                avg_variance = accelerator.gather(
                    variance.repeat(args.train_batch_size)
                ).mean()
                # avg_exp_variance = accelerator.gather(exp_variance.repeat(args.train_batch_size)).mean()

                train_loss += avg_loss.item() / args.gradient_accumulation_steps
                train_pretrain_loss += (
                    avg_pretrain_loss.item() / args.gradient_accumulation_steps
                )
                train_reward_loss += (
                    avg_reward_loss.item() / args.gradient_accumulation_steps
                )
                train_reward_wo_loss += (
                    avg_reward_wo_loss.item() / args.gradient_accumulation_steps
                )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    ):  # TODO: [c7w] finish logic for validation
            ...
            if global_step % args.checkpointing_steps == 0:
                if accelerator.distributed_type == accelerate.DistributedType.FSDP:
                    ...
                    if accelerator.is_main_process:
                        torch.save(
                            state_dict,
                            os.path.join(save_path, "controlnet_state_dict.pt"),
                        )
    ...
    if accelerator.distributed_type == accelerate.DistributedType.FSDP:
        with FSDP.state_dict_type(
            controlnet, StateDictType.FULL_STATE_DICT, full_state_dict_config
        ):
            state_dict = accelerator.get_state_dict(controlnet)
        ...
        if accelerator.is_main_process:
            torch.save(
                state_dict, os.path.join(args.output_dir, "controlnet_state_dict.pt")
            )
            torch.save(
                ema_state_dict,
                os.path.join(args.output_dir, "controlnet_state_dict_ema.pt"),
            )
