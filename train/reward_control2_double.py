...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--tune_sd", type=str, choices=["all", "decoder_only", "no"], default="no")
    parser.add_argument(
...
def main(args):
    ...
    elif args.tune_sd == "decoder_only":
        trainable_parameters += list(unet.up_blocks.parameters()) + list(unet.conv_out.parameters())
    else:
        pass
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        train_loss, train_pretrain_loss, train_reward_loss, train_reward_wo_loss, reward_wo_loss = (
            0.,
            0.,
            0.,
            0.,
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, controlnet):
                ...
                if args.rflow:
                    ...
                    denoised = model_pred * (-sigmas) + noisy_latents
                    ...
                    loss = torch.mean(
                        (weighting.float() * (denoised.float() - target.float()) ** 2).reshape(target.shape[0], -1),
                        1,
                    )
                    pretrain_loss = loss.mean()

                    # Map the denoised latents into RGB images
                    pred_original_sample = 1 / vae.config.scaling_factor * denoised
                    ...
                    reward_scale = min(1.0, global_step / args.linear_reward_scale_steps)
                    loss = pretrain_loss + reward_loss * args.grad_scale * reward_scale
                    ...
                    if args.uncertainty:
                        ...
                        if args.grad_scale == 0:
                            ...
                            # Determine which samples in the current batch need to calculate reward loss
                            timestep_mask = (args.min_timestep_rewarding <= timesteps.reshape(-1, 1)) & (timesteps.reshape(-1, 1) <= args.max_timestep_rewarding) 
                            timestep_mask1 = timestep_mask[: args.train_batch_size]
                            timestep_mask2 = timestep_mask[args.train_batch_size :]
                            # batch=2   [300,170]  -> [300,170,301,171] 
                            ...
                            variance1 = (torch.sum(kl_distance(uncertainty_log_sm(output_primary),uncertainty_sm(output_auxiliary),),
                                    dim=1,)* args.uncertainty_scale
                            )  # TODO check mean(variance) -> 0

                            variance2 = (torch.sum(kl_distance(uncertainty_log_sm(output_auxiliary),uncertainty_sm(output_primary),),
                                    dim=1,)* args.uncertainty_scale
                            )  # TODO check
                            ...
                            reward_wo_loss1 = get_reward_loss(output_primary, labels, args.task_name, reduction="none")  # B,W,H
                            reward_loss1 = reward_wo_loss1 * exp_variance1

                            exp_variance2 = torch.exp(-variance2)
                            reward_wo_loss2 = get_reward_loss(output_auxiliary, labels, args.task_name, reduction="none")  # B,W,H
                            reward_loss2 = reward_wo_loss2 * exp_variance2

                            # Reawrd Loss: (B, H, W)  =>  (B)
                            if args.task_name == "segmentation":
                                background_mask = (labels != 255).float()
                                # 1
                                reward_loss1 = (background_mask * reward_loss1).sum(
                                    dim=(-1, -2)
                                ) / (background_mask.sum(dim=(-1, -2)) + 1e-10)
                                
                                reward_wo_loss1 = (background_mask * reward_wo_loss1).sum(
                                    dim=(-1, -2)
                                ) / (background_mask.sum(dim=(-1, -2)) + 1e-10)
                                # 2
                                reward_loss2 = (background_mask * reward_loss2).sum(
                                    dim=(-1, -2)
                                ) / (background_mask.sum(dim=(-1, -2)) + 1e-10)
                                
                                reward_wo_loss2 = (background_mask * reward_wo_loss2).sum(
                                    dim=(-1, -2)
                                ) / (background_mask.sum(dim=(-1, -2)) + 1e-10)
                            elif args.task_name == "canny":
                                # 1
                                reward_loss1 = reward_loss1.mean(dim=(-1, -2))
                                reward_wo_loss1 = reward_wo_loss1.mean(dim=(-1, -2))
                                # 2
                                reward_loss2 = reward_loss2.mean(dim=(-1, -2))
                                reward_wo_loss2 = reward_wo_loss2.mean(dim=(-1, -2))
                            else:
                                # 1
                                reward_loss1 = reward_loss1.mean(dim=(-1, -2))
                                reward_wo_loss1 = reward_wo_loss1.mean(dim=(-1, -2))
                            ...
                            # 1
                            reward_loss1 = reward_loss1.reshape_as(timestep_mask1)
                            reward_loss1 = (timestep_mask1 * reward_loss1).sum() / (
                                timestep_mask1.sum() + 1e-10
                            )
                            
                            reward_wo_loss1 = reward_wo_loss1.reshape_as(timestep_mask1)
                            reward_wo_loss1 = (timestep_mask1 * reward_wo_loss1).sum() / (
                                timestep_mask1.sum() + 1e-10
                            )
                            # 2
                            reward_loss2 = reward_loss2.reshape_as(timestep_mask2)
                            reward_loss2 = (timestep_mask2 * reward_loss2).sum() / (
                                timestep_mask2.sum() + 1e-10
                            )
                            
                            reward_wo_loss2 = reward_wo_loss2.reshape_as(timestep_mask2)
                            reward_wo_loss2 = (timestep_mask2 * reward_wo_loss2).sum() / (
                                timestep_mask2.sum() + 1e-10
                            )
                            
                            # 1
                            variance1 = variance1.mean(dim=(-1, -2)).reshape_as(timestep_mask1)
                            variance1 = (timestep_mask1 * variance1).sum() / (
                                timestep_mask1.sum() + 1e-10
                            )
                            # 2
                            variance2 = variance2.mean(dim=(-1, -2)).reshape_as(timestep_mask2)
                            variance2 = (timestep_mask2 * variance2).sum() / (
                                timestep_mask2.sum() + 1e-10
                            )
                            
                            reward_scale = min(1.0, global_step / args.linear_reward_scale_steps)
                            
                            loss = pretrain_loss + (reward_loss1 + reward_loss2) * args.grad_scale * reward_scale + torch.mean(variance1) + torch.mean(variance2)
                    else:
                ...
                avg_reward_loss = accelerator.gather(reward_loss1.repeat(args.train_batch_size)).mean()
            ...
            logs = {
                ...
                "reward_loss_step": reward_loss1.detach().item(),
